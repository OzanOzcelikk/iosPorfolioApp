...
class DataController: ObservableObject {
    ...
    var suggesteedFilterTokens: [Tag]{
    ...
    func issuesForSelectedFilter() -> [Issue] {
        ...
        if let tag = filter.tag {
            ...
            let datePredicate = NSPredicate(format: "modeficationDate > %@", filter.minModificationDate as NSDate)
            predicates.append(datePredicate)
        }
        
        let trimmedFilterText = filter.name.trimmingCharacters(in: .whitespaces)
        ...
        if filterTokens.isEmpty == false {
            for filterToken in filterTokens {
                let tokenPredicate = NSPredicate( format: "tags IN %@", filterToken)
